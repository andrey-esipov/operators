...
export async function generateFighter(
  id: string,
  opts: { force?: boolean; heightCm?: number; publicDir?: string; log?: (s: string) => void } = {},
  ...
  const outcomes: FrameOutcome[] = [{
    ...
    rejects: [], warnings: [], metrics: { identity: 1, density: 0, coverage: 0, aspect: 0, footDrift: 0, bottomDrift: 0 },
  }]
  ...
  const results = await mapLimit(FRAMES, CONCURRENCY, async (spec) => {
    ...
    return { spec, best: best!, attempts, fromCache }
  })
...
async function main() {
  ...
  if (!id) throw new Error('usage: generate-animation-set.ts <fighterId> [--force]')
  const force = process.argv.includes('--force')
  const s = await generateFighter(id, { force })
  printSummary(s)
